...
Public Function PathGetAbsolute(psPath As String) As String
    PathGetAbsolute = String(MAX_PATH, Chr$(0))
    PathCanonicalize PathGetAbsolute, psPath & Chr$(0)
...
Public Function PathGetExtension(psPath As String) As String
    PathGetExtension = PathGetFileName(psPath)
    Dim liVal As Long
    liVal = Len(PathGetExtension) - InStrRev(PathGetExtension, ".")
...
Public Function PathGetRelative(ByVal psDirFrom As String, _
    ...
    Dim liPos As Long
    liPos = InStr(1, PathGetRelative, Chr$(0))
...
Public Function FileDelete(psPath As String, _
            Optional ByVal pbForceIfReadOnly As Boolean = True) _
                As Boolean
...
Public Function FileGetTempName(Optional psPath As String, Optional ByVal psPrefix As String = "tmp") As String
    If Len(psPath) = 0 Then psPath = PathGetSpecial(sfTemporary)
    If miUnique = 2147483647 Then miUnique = 1 Else miUnique = miUnique + 1
    PrepareString FileGetTempName
    GetTempFileName psPath, psPrefix, miUnique, FileGetTempName
    StripNulls FileGetTempName
End Function
...
Public Function FolderDelete(psPath As String, _
    ...
    For Each lvTemp In loColl
        If Not FolderDelete(CStr(lvTemp)) Then Exit Function
    Next
...
Public Function DriveGetStrings() As String()
    Dim lsStrings() As String
    Dim lsTemp As String
    On Error Resume Next
    lsTemp = String(MAX_PATH, 0)
    GetLogicalDriveStrings MAX_PATH, lsTemp
    lsTemp = Left$(lsTemp, InStr(1, lsTemp, Chr$(0) & Chr$(0)) - 1)
    lsStrings = Split(lsTemp, Chr$(0))
    DriveGetStrings = lsStrings
End Function
...
Public Function BrowseForFolder(piHwnd As Long, Optional psPrompt As String, Optional psPath As String) As String
    ...
    If lpItem Then
        ...
        If SHGetPathFromIDList(ByVal lpItem, sDir) Then
            BrowseForFolder = Left(sDir, InStr(1, sDir, Chr(0)) - 1)
        End If
    End If
    GlobalFree lpItem
    msInitFolder = ""
